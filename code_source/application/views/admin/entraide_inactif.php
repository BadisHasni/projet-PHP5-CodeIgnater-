			...
			xmlhttp.onreadystatechange=function() {
				if (xmlhttp.readyState==4 && xmlhttp.status==200) {
					bootbox.alert("L'Annonce a été supprimée", function() {
						window.open("<?php echo my_url('index.php/admin/');?>entraide_inactif", "_self");
					});
				}
			}
			xmlhttp.open("GET","<?php echo my_url('index.php/admin/');?>delete_annonce/"+i,true);
			xmlhttp.send();
		}
	})
	}
	...
	function statutAnnonce(i){
	  bootbox.confirm("Voulez-vous changer le Statut de cette Annonce ?",function(result) {
		if (result)
		{
			...
			xmlhttp.open("GET","<?php echo my_url('index.php/admin/');?>statut_annonce/"+i,true);
			xmlhttp.send();
		}
	})
	}
              ...
              <div class="col-lg-12">
                <div class="box">
                  <header>
                    ...
                    <h5>Liste des Entraide Inactifs</h5>
                  </header>
					<br/>
                          ...
                          if(isset($annonce)){
								...
								$row->desc = str_replace("'", "\'", $row->desc);
                                  ...
                                  echo  '<tr>
											...
											<td><button type="button" name="images" class="btn btn-info" onclick="showImg(\''.$row->img.'\')">Voir</button></td>
								  ...
								  echo 	'</td>
											<td>'.'<button id="delete_ann" onclick="statutAnnonce('.$row->id_annonce.')" type="button" class="btn btn-warning">Activer</button>'.'</td>
                                            <td>'.'<a href="'.my_url("index.php/admin/").'editer_annonce/'.$row->id_annonce.'/'.$row->id_region.'"><button type="button" class="btn btn-success">Editer</button></a>'.'</td>

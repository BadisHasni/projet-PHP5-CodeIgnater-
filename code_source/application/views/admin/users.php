...
<html class="no-js">
  <head>
    ...
    <script>
	  ...
	  bootbox.confirm("Voulez-vous supprimer cet Utilisateur ?",function(result) {
		if (result)
		{
			...
			xmlhttp.onreadystatechange=function() {
				if (xmlhttp.readyState==4 && xmlhttp.status==200) {
					bootbox.alert("L'Utilisateur a été supprimé", function() {
						window.open("<?php echo my_url('index.php/admin/');?>Users", "_self");
					});
				}
			}
			xmlhttp.open("GET","<?php echo my_url('index.php/admin/');?>delete_user/"+i,true);
			xmlhttp.send();
		}
	})
	}
	
	function statutUser(i){
	  bootbox.confirm("Voulez-vous changer le Statut de cet Utilisateur ?",function(result) {
		if (result)
		{
			if (window.XMLHttpRequest) {
				// code for IE7+, Firefox, Chrome, Opera, Safari
			...
			xmlhttp.open("GET","<?php echo my_url('index.php/admin/');?>statut_user/"+i,true);
			xmlhttp.send();
		}
	})
	}
                       ...
                       <?php 
                          if(isset($users)){
                                ...
                                foreach ($users->result() as $row) {
								  ...
								  echo 	'</td>
											<td>'.'<button id="delete_cat" onclick="statutUser('.$row->id_utilisateur.')" type="button" class="btn btn-warning">Changer Statut</button>'.'</td>
                                            <td>'.'<a href="'.my_url("index.php/admin/").'editer_user/'.$row->id_utilisateur.'/'.$row->id_region.'"><button type="button" class="btn btn-success">Editer</button></a>'.'</td>
                                            <td>'.'<button id="delete_cat" onclick="deleteUser('.$row->id_utilisateur.')" type="button" class="btn btn-danger">Supprimer</button>'.'</td>
                                        </tr>';
                                }

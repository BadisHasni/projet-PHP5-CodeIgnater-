...
<html class="no-js">
  <head>
    ...
    <script>
	  ...
	  function deleteAsso(i){
	  bootbox.confirm("Voulez-vous supprimer cette Association ?",function(result) {
		if (result)
		{
			...
			xmlhttp.onreadystatechange=function() {
				if (xmlhttp.readyState==4 && xmlhttp.status==200) {
					bootbox.alert("L'Association a été supprimé", function() {
						window.open("<?php echo my_url('index.php/admin/');?>Associations", "_self");
					});
				}
			}
			xmlhttp.open("GET","<?php echo my_url('index.php/admin/');?>delete_association/"+i,true);
			xmlhttp.send();
		}
	})
	}
	
	function statutAsso(i){
	  bootbox.confirm("Voulez-vous changer le Statut de cette Association ?",function(result) {
		if (result)
		{
			if (window.XMLHttpRequest) {
				// code for IE7+, Firefox, Chrome, Opera, Safari
			...
			xmlhttp.open("GET","<?php echo my_url('index.php/admin/');?>statut_association/"+i,true);
			xmlhttp.send();
		}
	})
	}
                      ...
                      <tbody>
                       ...
                       <?php 
                          if(isset($associations)){
                                ...
                                foreach ($associations->result() as $row) {
								  ...
								  echo 	'</td>
											<td>'.'<button id="delete_cat" onclick="statutAsso('.$row->id_utilisateur.')" type="button" class="btn btn-warning">Changer Statut</button>'.'</td>
                                            <td>'.'<a href="'.my_url("index.php/admin/").'editer_association/'.$row->id_utilisateur.'/'.$row->id_region.'"><button type="button" class="btn btn-success">Editer</button></a>'.'</td>
                                            <td>'.'<button id="delete_cat" onclick="deleteAsso('.$row->id_utilisateur.')" type="button" class="btn btn-danger">Supprimer</button>'.'</td>
                                        </tr>';
                                }

...
<html class="no-js">
  <head>
    ...
    <script>
	  ...
	  bootbox.confirm("Voulez-vous supprimer cette Annonce ?",function(result) {
		if (result)
		{
			...
			xmlhttp.onreadystatechange=function() {
				if (xmlhttp.readyState==4 && xmlhttp.status==200) {
					bootbox.alert("L'Annonce a été supprimée", function() {
						window.open("<?php echo my_url('index.php/test_index/');?>don_actif", "_self");
					});
				}
			}
			xmlhttp.open("GET","<?php echo my_url('index.php/test_index/');?>delete_annonce/"+i,true);
			xmlhttp.send();
		}
	})
	}
	
	function statutAnnonce(i){
	  bootbox.confirm("Voulez-vous changer le Statut de cette Annonce ?",function(result) {
		if (result)
		{
			if (window.XMLHttpRequest) {
				// code for IE7+, Firefox, Chrome, Opera, Safari
			...
			xmlhttp.open("GET","<?php echo my_url('index.php/test_index/');?>statut_annonce/"+i,true);
			xmlhttp.send();
		}
	})
	}
	...
	function showDesc(desc){
		new Messi(desc, {title: 'Description', modal: true});
	}
	
	function showImg(img){
		Messi.img("<?php echo my_url('assets/img/annonces/');?>"+img, {modal: true});
	}
	...
	<!-- Messi -->
	<link rel="stylesheet" href="<?php echo my_url("assets/lib/jquery.messi/css/messi.css");?>" />
    ...
    <div class="bg-dark dk" id="wrap" style="margin=0px;">
      ...
      <div id="left">
        ...
        <ul id="menu" class="bg-blue dker">
          ...
          <li class="">
            <a href="<?php echo base_url().'index.php/test_index/';?>">
        ...
        <li class="">
			...
			<ul>
				<li>
					...
					<ul>
						<li> <a href="<?php echo my_url('index.php/test_index/');?>troc_actif">Active</a>  </li>
						<li> <a href="<?php echo my_url('index.php/test_index/');?>troc_inactif">Inactive</a>  </li>
					</ul>
				</li>
				<li>
					<a href="javascript:;">Don  <span class="fa arrow"></span>  </a> 
					<ul>
						<li> <a href="<?php echo my_url('index.php/test_index/');?>don_actif">Active</a>  </li>
						<li> <a href="<?php echo my_url('index.php/test_index/');?>don_inactif">Inactive</a>  </li>
					</ul>
				</li>
				<li>
					<a href="javascript:;">Entraide  <span class="fa arrow"></span>  </a> 
					<ul>
						<li> <a href="<?php echo my_url('index.php/test_index/');?>entraide_actif">Active</a>  </li>
						<li> <a href="<?php echo my_url('index.php/test_index/');?>entraide_inactif">Inactive</a>  </li>
					</ul>
				</li>
			</ul>
		</li>
          ...
          <li>
            <a href="<?php echo my_url('index.php/test_index/');?>categorie">
           ...
           <li class="">
            ...
            <ul> 
                <li>
                    <a href="<?php echo my_url('index.php/test_index/');?>Users">
                      <i class="fa fa-angle-right"></i>&nbsp; Utilisateurs</a> 
                    </li>
				<li>
                    <a href="<?php echo my_url('index.php/test_index/');?>Associations">
          ...
          <li>
            <a href="<?php echo my_url('index.php/test_index/');?>Admins">
...
<div class="row">
                      ...
                      <tbody>
                       ...
                       <?php 
                          if(isset($annonce)){
                                ...
                                foreach ($annonce->result() as $row) {
								$row->desc = str_replace("'", "\'", $row->desc);
                                  
                                  echo  '<tr>
                                            
                                            <td>'.$row->titre.'</td>
											<td><button type="button" name="description" class="btn btn-info" onclick="showDesc(\''.$row->desc.'\')">Lire</button></td>
											<td><button type="button" name="photo" class="btn btn-info" onclick="showImg(\''.$row->img.'\')">Voir</button></td>
                                            <td>'.$row->date_insertion.'</td>
                                            <td>'.$row->login.'</td>
                                            <td>';
												if ($row->statut == 1)
								  ...
								  echo 	'</td>
											<td>'.'<button id="delete_ann" onclick="statutAnnonce('.$row->id_annonce.')" type="button" class="btn btn-warning">Désactiver</button>'.'</td>
                                            <td>'.'<a href="'.my_url("index.php/test_index/").'editer_annonce/'.$row->id_annonce.'/'.$row->id_region.'"><button type="button" class="btn btn-success">Editer</button></a>'.'</td>
                                            <td>'.'<button id="delete_ann" onclick="deleteAnnonce('.$row->id_annonce.')" type="button" class="btn btn-danger">Supprimer</button>'.'</td>
                                        </tr>';
                                }
	...
	<!--Messi -->
	<script src=<?php echo my_url("assets/lib/jquery.messi/js/messi.js");?> ></script>
